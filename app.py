...
# --- 5. 三大功能分頁 ---
tab1, tab2, tab3 = st.tabs(["🔤 A-Z 字母練習區", "📖 萬能隨機短文", "🎮 聽音辨圖遊戲"])

with tab1:
    st.header("🔤 字母與單字發音練習")
    sel_let = st.selectbox("請選擇字母練習", list(FULL_DB.keys()), key="alphabet_panel_final")
    ...
    for w, emo, sent, tr in info["words"]:
        ...
        with col2:
            ...
            if st.button(f"🔊 聽 {w} 發音", key=f"btn_tab1_v3_{w}"):
...
with tab2:
    ...
    if st.button("🎲 隨機生成新課文", use_container_width=True, key="random_gen_v3"):
        pick = random.choice(master_topics)
        t_cn, t_en, mode = pick["cn"], pick["en"], pick["type"]
        
        # 根據模式生成連貫文章
        if mode == "Animal":
            pool = [(f"I see the {t_en}.", f"我看到了{t_cn}。"), (f"They are very strong.", f"牠們非常強壯。"), (f"It is fun.", f"這很有趣。")]
        elif mode == "Nature":
            pool = [(f"The {t_en} is beautiful.", f"{t_cn}很美麗。"), (f"We can see it today.", f"我們今天可以看到它。")]
        else:
            pool = [(f"I like {t_en}.", f"我喜歡{t_cn}。"), (f"It is very cool.", f"這非常酷。")]
    ...
    if st.session_state.active_en_list:
        ...
        if st.button("🔊 全文朗讀", key="read_all_v3"):
...
with tab3:
    ...
    # 全部單字與 emoji 對照池
    full_pool = []
    for letter in FULL_DB:
        for w_tuple in FULL_DB[letter]["words"]:
            full_pool.append(w_tuple) # 格式為 (Apple, 🍎, Sentence, Translation)
    
    # 初始化遊戲狀態
    if f"q_ans_{st.session_state.game_turn}" not in st.session_state:
        # 隨機挑選三個不同的單字 Tuple
        game_items = random.sample(full_pool, 3)
        st.session_state[f"g_items_{st.session_state.game_turn}"] = game_items
        # 隨機指定其中一個為答案
        q_ans = random.choice(game_items)
        st.session_state[f"q_ans_{st.session_state.game_turn}"] = q_ans

    # 獲取當前回合的題目
    current_items = st.session_state[f"g_items_{st.session_state.game_turn}"]
    current_ans = st.session_state[f"q_ans_{st.session_state.game_turn}"]

    st.write(f"### 回合：{st.session_state.game_turn + 1}")
    if st.button("🔊 播放題目單字音效", key=f"play_audio_{st.session_state.game_turn}"):
        st.audio(get_voice(current_ans[0], voice_speed), format="audio/mp3")
    ...
    for i, item in enumerate(current_items):
        word, emoji, sent, tr = item
        with cols[i]:
            # 顯示超大 emoji 圖片
            st.markdown(f"<h1 style='text-align:center; font-size:180px; margin:0;'>{emoji}</h1>", unsafe_allow_html=True)
            # 圖片下方的選擇按鈕
            if st.button(f"這是 {word} 嗎？", key=f"choice_{st.session_state.game_turn}_{i}", use_container_width=True):
                if word == current_ans[0]:
                    st.success("🎉 太棒了！答對了！恐龍進化了！")
                    st.balloons()
                    st.session_state.user_score += 10
                    # 進入下一回合
                    st.session_state.game_turn += 1
                    time.sleep(1) # 暫停一下讓孩子看庆祝
                    st.rerun() # 刷新畫面生成新題目
                else:
                    st.error("❌ 糟糕，選錯圖片了，再聽一次音效試試看！")
